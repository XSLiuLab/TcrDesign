...
from ..model import BERTLM_pMHC, BERT
from .optim_schedule import ScheduledOptim

import tqdm

class BERTTrainer_pMHC:
    def __init__(self, bert: BERT, vocab_size: int,
        ...
        self.model = BERTLM_pMHC(self.bert, vocab_size).to(self.device)
        ...
        self.bse_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([5.0]).to(self.device)) # 正负样本不平衡，负：正 = 7.431561
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in data_iter:
            ...
            binding_score_output, mask_lm_output = self.model.forward(data["bert_input"], data["segment_label"])

            # 两个损失差距太大，加权以均衡
            binding_loss = self.bse_loss(binding_score_output.squeeze(), data["binding_score"])*4
            mask_loss = self.criterion(mask_lm_output.transpose(1, 2), data["bert_label"])*1
            loss = (binding_loss + mask_loss) / 2
            ...
            avg_loss += loss.item()
            avg_binding_loss += binding_loss.item()
            ...
            post_fix = {
                ...
                "avg_binding_loss": avg_binding_loss / (i + 1),
            ...
            if self.log_dir is not None:
                ...
                self.writer.add_scalar(f'pretrain/{str_code}_bindingLoss_step', binding_loss.item(), 
                                       epoch*self.batch_num+i if flag else epoch*len(data_iter)+i)
                self.writer.add_scalar(f'pretrain/{str_code}_maskLoss_step', mask_loss.item(), 
                                       epoch*self.batch_num+i if flag else epoch*len(data_iter)+i)
        ...
        if self.log_dir is not None:
            ...
            self.writer.add_scalar(f'pretrain/{str_code}_bindingLoss_epoch', 
                                   avg_binding_loss / self.batch_num if flag else avg_binding_loss / len(data_iter), epoch)
            self.writer.add_scalar(f'pretrain/{str_code}_maskLoss_epoch', 
                                   avg_mask_loss / self.batch_num if flag else avg_mask_loss / len(data_iter), epoch)
